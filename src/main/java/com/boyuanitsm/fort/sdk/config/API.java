package com.boyuanitsm.fort.sdk.config;

/**
 * API url constants.
 *
 * @author zhanghua on 5/26/16.
 */
public class API {

    public static final String SECURITY_USER_LOGOUT = "/api/security-user-logout";
    public static final String SECURITY_USER_BY_LOGIN = "/api/security-user-by-login";
    public static final String AUTHENTICATION = "/api/authentication";
    public static final String SA_SECURITY_RESOURCE_ENTITIES = "/api/sa/security-resource-entities";
    public static final String SA_SECURITY_AUTHORITIES = "/api/sa/security-authorities";
    public static final String SA_SECURITY_ROLES = "/api/sa/security-roles";
    public static final String SECURITY_USER_AUTHORIZATION = "/api/security-user/authorization";
    public static final String SECURITY_USERS_DTO = "/api/security-user-dto";
    public static final String SECURITY_USERS = "/api/security-users";
    public static final String SECURITY_GROUPS = "/api/security-groups";
    public static final String SECURITY_RESOURCE_ENTITIES = "/api/security-resource-entities";
    public static final String SECURITY_AUTHORITIES = "/api/security-authorities";
    public static final String SECURITY_NAVS = "/api/security-navs";
    public static final String SECURITY_ROLES = "/api/security-roles";

}
